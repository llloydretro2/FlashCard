...
_tg_screen_functions = [
    'addshape', 'bgcolor', 'bgpic', 'bye', 'clearscreen', 'colormode', 'delay',
    'exitonclick', 'getcanvas', 'getshapes', 'listen', 'mainloop', 'mode',
    'numinput', 'onkey', 'onkeypress', 'onkeyrelease', 'onscreenclick',
    'ontimer', 'register_shape', 'resetscreen', 'screensize', 'setup',
    'setworldcoordinates', 'textinput', 'title', 'tracer', 'turtles', 'update',
    'window_height', 'window_width'
]
_tg_turtle_functions = [
    'back', 'backward', 'begin_fill', 'begin_poly', 'bk', 'circle', 'clear',
    'clearstamp', 'clearstamps', 'clone', 'color', 'degrees', 'distance',
    'dot', 'down', 'end_fill', 'end_poly', 'fd', 'fillcolor', 'filling',
    'forward', 'get_poly', 'getpen', 'getscreen', 'get_shapepoly', 'getturtle',
    'goto', 'heading', 'hideturtle', 'home', 'ht', 'isdown', 'isvisible',
    'left', 'lt', 'onclick', 'ondrag', 'onrelease', 'pd', 'pen', 'pencolor',
    'pendown', 'pensize', 'penup', 'pos', 'position', 'pu', 'radians', 'right',
    'reset', 'resizemode', 'rt', 'seth', 'setheading', 'setpos', 'setposition',
    'settiltangle', 'setundobuffer', 'setx', 'sety', 'shape', 'shapesize',
    'shapetransform', 'shearfactor', 'showturtle', 'speed', 'st', 'stamp',
    'tilt', 'tiltangle', 'towards', 'turtlesize', 'undo', 'undobufferentries',
    'up', 'width', 'write', 'xcor', 'ycor'
]
...
_alias_list = [
    'addshape', 'backward', 'bk', 'fd', 'ht', 'lt', 'pd', 'pos', 'pu', 'rt',
    'seth', 'setpos', 'setposition', 'st', 'turtlesize', 'up', 'width'
]
...
__stringBody = ('def %(method)s(self, *args, **kw): return ' +
...
class ScrolledCanvas(TK.Frame):
    ...
    def __init__(self,
        ...
        self._canvas = TK.Canvas(master,
                                 ...
                                 relief=TK.SUNKEN,
                                 borderwidth=2)
        self.hscroll = TK.Scrollbar(master,
    ...
    def adjustScrolls(self):
        ...
        self._canvas.xview_moveto(0.5 * (self.canvwidth - cwidth) /
                                  self.canvwidth)
        self._canvas.yview_moveto(0.5 * (self.canvheight - cheight) /
                                  self.canvheight)
...
##        """may be implemented for some other graphics toolkit"""
    ...
    def _onrelease(self, item, fun, num=1, add=None):
        ...
        if fun is None:
            ...
            self.cv.tag_bind(item, "<Button%s-ButtonRelease>" % num, eventfun,
                             add)
    ...
    def numinput(self, title, prompt, default=None, minval=None, maxval=None):
        ...
        """
        return simpledialog.askfloat(title,
                                     prompt,
...
class TurtleScreen(TurtleScreenBase):
    ...
    def __init__(self,
        ...
        self._shapes = {
            "arrow":
            Shape("polygon", ((-10, 0), (10, 0), (0, 10))),
            "turtle":
            Shape("polygon",
                  ((0, 16), (-2, 14), (-1, 10), (-4, 7), (-7, 9), (-9, 8),
                   (-6, 5), (-7, 1), (-5, -3), (-8, -6), (-6, -8), (-4, -5),
                   (0, -7), (4, -5), (6, -8), (8, -6), (5, -3), (7, 1), (6, 5),
                   (9, 8), (7, 9), (4, 7), (1, 10), (2, 14))),
            "circle":
            Shape("polygon",
            ...
            "triangle":
            Shape("polygon", ((10, -5.77), (0, 11.55), (-10, -5.77))),
            "classic":
            Shape("polygon", ((0, 0), (-5, -9), (0, -7), (5, -9))),
            "blank":
            Shape("image", self._blankimage())
        }
...
class TNavigator(object):
    ...
    def towards(self, x, y=None):
        ...
        return (self._angleOffset +
                self._angleOrient * result) % self._fullcircle
...
class TPen(object):
    ...
    def pen(self, pen=None, **pendict):
        ...
        if "stretchfactor" in p or "tilt" in p or "shearfactor" in p:
            ...
            self._shapetrafo = (scx * ca, scy * (shf * ca + sa), -scx * sa,
                                scy * (ca - shf * sa))
        self._update()
...
class _TurtleImage(object):
    ...
    def _setshape(self, shapeIndex):
        ...
        if self._type == "polygon":
            ...
            self._item = [
                screen._createpoly()
                for item in screen._shapes[shapeIndex]._data
            ]
...
class RawTurtle(TPen, TNavigator):
    ...
    def _polytrafo(self, poly):
        ...
        e0, e1 = (1.0 / abs(e)) * e
        return [(p0 + (e1 * x + e0 * y) / screen.xscale,
                 p1 + (-e0 * x + e1 * y) / screen.yscale) for (x, y) in poly]
    ...
    def _getshapepoly(self, polygon, compound=False):
        ...
        if self._resizemode == "user" or compound:
            ...
            return polygon
        return tuple(
            (t11 * x + t12 * y, t21 * x + t22 * y) for (x, y) in polygon)
...
##############################  stamp stuff  ###############################
    ...
    def _goto(self, end):
        ...
        undo_entry = ("go", self._position, end, go_modes,
                      (self.currentLineItem, self.currentLine[:],
                       screen._pointlist(self.currentLineItem), self.items[:]))
        if self.undobuffer:
        ...
        if self._speed and screen._tracing == 1:
            ...
            diffsq = (diff[0] * screen.xscale)**2 + (diff[1] *
                                                     screen.yscale)**2
            nhops = 1 + int(
                (diffsq**0.5) / (3 * (1.1**self._speed) * self._speed))
            ...
            for n in range(1, nhops):
                ...
                if self._drawing:
                    screen._drawline(self.drawingLineItem,
                                     (start, self._position), self._pencolor,
                                     self._pensize, top)
                self._update()
            if self._drawing:
    ...
    def _undogoto(self, entry):
        ...
        todelete = [
            ...
            if (i not in items) and (screen._type(i) == "line")
        ]
        for i in todelete:
            screen._delete(i)
            self.items.remove(i)
    ...
    def end_fill(self):
        ...
        if self.filling():
            if len(self._fillpath) > 2:
                self.screen._drawpoly(self._fillitem,
                                      self._fillpath,
...
def _make_global_funcs(functions, cls, obj, init, docrevise):
    for methodname in functions:
        ...
        if pl1 == "":
            print(">>>>>>", pl1, pl2)
            continue
        defstr = __func_body.format(obj=obj,
                                    init=init,
                                    name=methodname,
                                    paramslist=pl1,
                                    argslist=pl2)
