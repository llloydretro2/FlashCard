...
class Stats:
    ...
    def init(self, arg):
        ...
        try:
            ...
            print("Invalid timing data %s" %
                  (self.files[-1] if self.files else ''),
                  file=self.stream)
            raise
    ...
    def get_stats_profile(self):
        ...
        """
        func_list = self.fcn_list[:] if self.fcn_list else list(
            self.stats.keys())
        if not func_list:
        ...
        for func in clist:
            ...
            if isinstance(value, tuple):
                ...
                if nc != cc:
                    ...
                    substats = '%d' % (nc, )
                substats = '%s %s %s  %s' % (
                    substats.rjust(7 + 2 * len(indent)), f8(tt), f8(ct), name)
    ...
    class ProfileBrowser(cmd.Cmd):
        ...
        def generic(self, fn, line):
            ...
            for term in args:
                ...
                try:
                    frac = float(term)
                    if frac > 1 or frac < 0:
                        print("Fraction argument must be in [0, 1]",
                              file=self.stream)
                        continue
        ...
        def do_add(self, line):
            if self.stats:
                try:
                    ...
                    print("Failed to load statistics for %s: %s" % (line, e),
                          file=self.stream)
            else:
                print("No statistics object is loaded.", file=self.stream)
            return 0
        ...
        def help_read(self):
            print("Read in profile data from a specified file.",
                  file=self.stream)
            print("Without argument, reload the current file.",
                  file=self.stream)
        ...
        def complete_sort(self, text, *args):
            return [
                a for a in Stats.sort_arg_dict_default if a.startswith(text)
            ]
