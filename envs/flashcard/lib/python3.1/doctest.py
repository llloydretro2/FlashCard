...
class Example:
    ...
    def __hash__(self):
        return hash(
            (self.source, self.want, self.lineno, self.indent, self.exc_msg))
...
class DocTestParser:
    ...
    def get_doctest(self, string, globs, name, filename, lineno):
        ...
        """
        return DocTest(self.get_examples(string, name), globs, name, filename,
                       lineno, string)
    ...
    def _find_options(self, source, name, lineno):
        ...
        for m in self._OPTION_DIRECTIVE_RE.finditer(source):
            ...
            for option in option_strings:
                if (option[0] not in '+-'
                        or option[1:] not in OPTIONFLAGS_BY_NAME):
    ...
    def _find(self, tests, obj, name, module, source_lines, globs, seen):
        ...
        if inspect.ismodule(obj) and self._recurse:
            for valname, val in obj.__dict__.items():
                ...
                if ((self._is_routine(val) or inspect.isclass(val))
        ...
        if inspect.ismodule(obj) and self._recurse:
            for valname, val in getattr(obj, '__test__', {}).items():
                ...
                if not (inspect.isroutine(val) or inspect.isclass(val)
                        or inspect.ismodule(val) or isinstance(val, str)):
        ...
        if inspect.isclass(obj) and self._recurse:
            for valname, val in obj.__dict__.items():
                ...
                if ((inspect.isroutine(val) or inspect.isclass(val)
    ...
    def __run(self, test, compileflags, out):
        ...
        for examplenum, example in enumerate(test.examples):
            ...
            try:
                ...
                exec(
                    compile(example.source, filename, "single", compileflags,
                            True), test.globs)
