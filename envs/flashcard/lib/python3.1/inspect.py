...
def classify_class_attrs(cls):
    ...
    for base in mro:
        for k, v in base.__dict__.items():
            if isinstance(v,
                          types.DynamicClassAttribute) and v.fget is not None:
                names.append(k)
    result = []
    processed = set()
    ...
    for name in names:
        ...
        if isinstance(dict_obj, (staticmethod, types.BuiltinMethodType)):
            kind = "static method"
            obj = dict_obj
        elif isinstance(dict_obj,
                        (classmethod, types.ClassMethodDescriptorType)):
            kind = "class method"
            obj = dict_obj
...
def _finddoc(obj):
    ...
    if ismethod(obj):
        ...
        self = obj.__self__
        if (isclass(self) and getattr(getattr(self, name, None), '__func__')
                is obj.__func__):
            # classmethod
            cls = self
        else:
...
def getsourcefile(object):
    ...
    if getattr(module, '__loader__', None) is not None:
        return filename
    elif getattr(getattr(module, "__spec__", None), "loader",
                 None) is not None:
        return filename
...
def getmodule(object, _filename=None):
    ...
    for modname, module in sys.modules.copy().items():
        if ismodule(module) and hasattr(module, '__file__'):
            ...
            modulesbyfile[f] = modulesbyfile[os.path.realpath(
                f)] = module.__name__
    if file in modulesbyfile:
        return sys.modules.get(modulesbyfile[file])
...
def getsourcelines(object):
    ...
    if (ismodule(object)
            or (isframe(object) and object.f_code.co_name == "<module>")):
        return lines, 0
    else:
...
def _missing_arguments(f_name, argnames, pos, values):
    ...
    raise TypeError("%s() missing %i required %s argument%s: %s" %
                    (f_name, missing, "positional" if pos else "keyword-only",
...
def getcallargs(func, /, *positional, **named):
    ...
    if num_pos > num_args and not varargs:
        _too_many(f_name, args, kwonlyargs, varargs, num_defaults, num_pos,
                  arg2value)
    if num_pos < num_args:
...
Traceback = namedtuple('Traceback',
...
_NonUserDefinedCallables = (_WrapperDescriptor, _MethodWrapper,
                            _ClassMethodWrapper, types.BuiltinFunctionType)
...
def _signature_is_builtin(obj):
    ...
    """
    return (
        isbuiltin(obj) or ismethoddescriptor(obj)
        or isinstance(obj, _NonUserDefinedCallables) or
...
def _signature_is_functionlike(obj):
    ...
    defaults = getattr(obj, '__defaults__',
                       _void)  # Important to use _void ...
    kwdefaults = getattr(obj, '__kwdefaults__', _void)  # ... and not None here
    annotations = getattr(obj, '__annotations__', None)

    return (isinstance(code, types.CodeType) and isinstance(name, str)
...
def _signature_from_function(cls,
                             func,
                             skip_bound_arg=True,
                             globals=None,
                             locals=None,
...
def _signature_from_callable(obj,
    ...
    try:
        ...
        if sig is not None:
            if not isinstance(sig, Signature):
                raise TypeError('unexpected object {!r} in __signature__ '
    ...
    try:
        ...
        if isinstance(partialmethod, functools.partialmethod):
            ...
            if first_wrapped_param.kind is Parameter.VAR_POSITIONAL:
                ...
                assert (not sig_params
                        or first_wrapped_param is not sig_params[0])
    ...
    if isfunction(obj) or _signature_is_functionlike(obj):
        ...
        return _signature_from_function(sigcls,
                                        obj,
                                        skip_bound_arg=skip_bound_arg,
                                        globals=globals,
                                        locals=locals,
    ...
    if _signature_is_builtin(obj):
        return _signature_from_builtin(sigcls,
                                       obj,
                                       skip_bound_arg=skip_bound_arg)
        ...
        if sig is None:
            ...
            if type not in obj.__mro__:
                ...
                if (obj.__init__ is object.__init__
                        and obj.__new__ is object.__new__):
...
class Parameter:
    ...
    * kind : str
    ...
    def __init__(self, name, kind, *, default=_empty, annotation=_empty):
        ...
        if name[0] == '.' and name[1:].isdigit():
            ...
            if self._kind != _POSITIONAL_OR_KEYWORD:
                msg = ('implicit arguments must be passed as '
    ...
    def __eq__(self, other):
        ...
        return (self._name == other._name and self._kind == other._kind
...
class Signature:
    ...
    * bind_partial(*args, **kwargs) -> BoundArguments
        ...
        if parameters is None:
            ...
            if __validate_parameters__:
                ...
                for param in parameters:
                    ...
                    if kind < top_kind:
                        msg = ('wrong parameter order: {} parameter before {} '
                               'parameter')
    ...
    def from_function(cls, func):
        ...
        warnings.warn(
            "inspect.Signature.from_function() is deprecated since "
    ...
    def from_builtin(cls, func):
        ...
        warnings.warn(
            "inspect.Signature.from_builtin() is deprecated since "
            "Python 3.5, use Signature.from_callable()",
            DeprecationWarning,
            stacklevel=2)
